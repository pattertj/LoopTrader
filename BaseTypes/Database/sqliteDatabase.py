...
import sqlite3
from sqlite3.dbapi2 import Connection, Cursor, IntegrityError
...
class SqliteDatabase(Database):
    ...
    def create_strategy(self, strategy_class: str, name: str, underlying: str) -> bool:
        ...
        try:
            result = self.connection.execute(query, (strategy_class, name, underlying))
            self.connection.commit()
        except IntegrityError:
            logger.warning("Strategy already exists")
            # Read the Strat
            # Return the ID
        except Exception:
    ...
    def create_order(self, request: baseRR.DBCreateOrderRequestMessage) -> bool:
    ...
    def pre_flight_db_check(self):
        ...
        if not strategiesexist:
            createorderstable = '''CREATE TABLE Strategies(StrategyID INTEGER PRIMARY KEY, StrategyClass TEXT, StrategyName TEXT, Underlying TEXT, UNIQUE(StrategyClass, StrategyName, Underlying))'''
            self.cursor.execute(createorderstable)
